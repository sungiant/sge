...
void update (sge::app::response& r, const sge::app::api& sge) {
    ...
    if (!r.uniforms_changed[0] && !r.uniforms_changed[1] && !push.no_change) {
        push.no_change = true;
        r.push_constants_changed = true;
    }

    if ((r.uniforms_changed[0] || r.uniforms_changed[1] || sge.runtime.system__did_container_just_change ()) && push.no_change) {
        push.no_change = false;
        r.push_constants_changed = true;
    }
